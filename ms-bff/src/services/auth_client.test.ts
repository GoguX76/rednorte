...
describe("AuthClient Service (BFF)", () => {
    it("Si las credenciales son inválidas, lanzará error 401", async () => {
        ...
        await expect(AuthClient.login("user_malo", "contra_mala")).rejects.toThrow("Credenciales inválidas o error en el servidor")
    ...
    it("De ser un login exitoso, debe devolver un JSON con los datos del usuario", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('http://localhost:8081/api/auth/users', {
            ...
            headers: { "Content-Type": "application/json" }, // Indica que el contenido es un JSON
            body: JSON.stringify({ username: "medico_rednorte", password: "medico12345" }) // Toma los datos y los convierte en JSON con stringify
        });
        mockFetch.mockRestore();
    });
});