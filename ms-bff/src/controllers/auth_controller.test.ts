...
describe("Controlador de Autenticación (BFF)", () => {
    it("Si AuthClient devuelve un JSON con los datos, el resultado será éxitoso (status 200)", async () => {
        ...
        const falseContext = {
            req: {
                json: async () => ({username: "medico_rednorte", password: "medico12345"}) // Credenciales válidas para resultado éxitoso
            },
    ...
    it("Si AuthClient devuelve error, el resultado dará error (status 401)", async () => {
        ...
        const mockLogin = spyOn(AuthClient, "login").mockRejectedValue(new Error("Credenciales inválidas o error en el servidor"));

        // Falsificamos el contexto para darle crendenciales inválidas al controlador
        const falseContext = {
            req: {
                json: async () => ({username: "user_invalido", password: "pass_invalida"})
            },
            json: (data: any, status: number) => {
                return {body: data, status: status}
            }
        ...
        expect(mockLogin).toHaveBeenCalledWith("user_invalido", "pass_invalida")
        // Limpiamos el Mock
        mockLogin.mockRestore();
    });
});